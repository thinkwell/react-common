...
export default function ResourceList(props) {
    ...
    return (_jsxs(LegacyStack, { vertical: true, children: [props.fetchItemsState && props.fetchItemsState.error ? _jsx(InlineError, { message: props.fetchItemsState && props.fetchItemsState.error, fieldID: "resourceListError" }) : null, props.fetchItemsError ? _jsx(InlineError, { message: props.fetchItemsError, fieldID: "resourceListError" }) : null, _jsx(LegacyCard, { children: _jsx(ResourceListShopify, { selectedItems: props.selectable && form.field && form.field(props.name), onSelectionChange: props.selectable && form.onData && form.onData(props.name), filterControl: props.filterControl, alternateTool: props.alternateTool, selectable: props.selectable, resourceName: resourceName, emptyState: emptyStateMarkup, items: itemsSorted, renderItem: props.renderItem, sortValue: order, sortOptions: sortOptions, onSortChange: handleSortChange, loading: props.fetchItemsState && props.fetchItemsState.loading || props.fetchItemsLoading }) }), paginationMarkup] }));
}
